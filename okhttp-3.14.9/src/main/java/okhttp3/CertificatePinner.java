...
public final class CertificatePinner {
  ...
  public void check(String hostname, List<Certificate> peerCertificates)
    ...
    if (pins.isEmpty()) return;// 如果该域名没有找到任何证书锁定的信息，则直接返回，即表示客户端没有进行证书锁定

    if (certificateChainCleaner != null) {
      peerCertificates = certificateChainCleaner.clean(peerCertificates, hostname);
    }
    // 遍历服务器返回的证书，并与客户端锁定的证书信息进行比较，主要是校验证书的 公钥PIN-SHA256 字段
    // 这也是客户端进行证书锁定的常用方式
    for (int c = 0, certsSize = peerCertificates.size(); c < certsSize; c++) {
      ...
      for (int p = 0, pinsSize = pins.size(); p < pinsSize; p++) {
        Pin pin = pins.get(p);
        // 客户端锁定的是证书的 pin-sha256 字段
        if (pin.hashAlgorithm.equals("sha256/")) {
          if (sha256 == null) sha256 = sha256(x509Certificate); // 获取服务器返回的证书的Sha256信息
          //  如果服务器证书的sha256信息，与客户端锁定的sha256信息相等，说明证书没有被篡改
          if (pin.hash.equals(sha256)) return; // Success!，
        } else if (pin.hashAlgorithm.equals("sha1/")) {// 锁定的是证书的 sha1指纹
          if (sha1 == null) sha1 = sha1(x509Certificate);
          if (pin.hash.equals(sha1)) return; // Success!
        } else {// 如果锁定的不是 sha1，也不是sha256，则报错，不支持其它锁定方式
  ...
  static final class Pin {
    ...
    /** 主机名 A hostname like {@code example.com} or a pattern like {@code *.example.com}. */
    final String pattern;
    /** The canonical hostname, i.e. {@code EXAMPLE.com} becomes {@code example.com}. */
    final String canonicalHostname;
    /** 或者sha1/或者sha256/ Either {@code sha1/} or {@code sha256/}. */
  ...
  /** 构建已配置的固定证书  Builds a configured certificate pinner. */
