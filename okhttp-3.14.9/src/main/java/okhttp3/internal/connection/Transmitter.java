...
public final class Transmitter {
  ...
  private Address createAddress(HttpUrl url) {
    ...
    if (url.isHttps()) {// 如果是Https
      sslSocketFactory = client.sslSocketFactory();// ssl工厂
      hostnameVerifier = client.hostnameVerifier();// 域名认证
      certificatePinner = client.certificatePinner();// 证书锁定
    }
    // 封装Address对象，包含有：域名，端口号，dns解析，socket工厂，ssl工厂，域名校验，证书锁定等
  ...
  Exchange newExchange(Interceptor.Chain chain, boolean doExtensiveHealthChecks) {
    ...
    // 查找一个可用的 数据交换编码器：加密请求，解密响应
    // 在这个过程中，查找可用的连接，并建立连接，获取输入流，输出流以及证书校验等。
    ...
    synchronized (connectionPool) {
      ...
      this.exchangeRequestDone = false;// 发送完成
      this.exchangeResponseDone = false;// 响应完成
      return result;// 返回数据交换机
    }
  }
  ...
  /**
   * 从连接分配的结合中，移除发射器。并返回一个Socket对象，调用者应该关闭这个Socket
  ...
  @Nullable Socket releaseConnectionNoEvents() {
    ...
    released.transmitters.remove(index);// 从集合中移除当前的发射器
    this.connection = null;

    // 如果连接上没有有数据流了，就置为空闲（等待清理），并返回待关闭的socket
    if (released.transmitters.isEmpty()) {// 连接的发射器集合为空时，
