...
public final class Http1ExchangeCodec implements ExchangeCodec {
  ...
  @Override public Source openResponseBodySource(Response response) {
    ...
    if (contentLength != -1) {
      //  返回一个 FixedLengthSource 对象
      return newFixedLengthSource(contentLength);
    }
    // 当内容长度不确定的时候，返回一个 UnknownLengthSource 对象
  ...
  @Override public Response.Builder readResponseHeaders(boolean expectContinue) throws IOException {
    ...
    try {
      ...
      Response.Builder responseBuilder = new Response.Builder()
          .protocol(statusLine.protocol)// 协议
          .code(statusLine.code) // 请求返回的状态
          .message(statusLine.message)// 状态信息
          .headers(readHeaders());// 请求头
  ...
  private Sink newKnownLengthSink() {
    if (state != STATE_OPEN_REQUEST_BODY) throw new IllegalStateException("state: " + state);
    state = STATE_WRITING_REQUEST_BODY;// 当前状态，写入请求体
  ...
  private abstract class AbstractSource implements Source {
    ...
    final void responseBodyComplete() {
      if (state == STATE_CLOSED) return;// 关闭状态
